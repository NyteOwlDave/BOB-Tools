...
/*
	...
	Modified to read stdin and stdout i.e.
...
void read_scan(FILE *fp, int index);
void old_write_scan(FILE *fp, char *buf[3]);
void write_scan(FILE *fp, char *buf[3]);
...
int main(int ac, char **av) {
	...
	write_scan(outfp, (char**)(inbuf[0]));    /* output 1st scan line */
	...
	for (y=1; y<yres-1; y++) {
		...
		write_scan(outfp, (char**)outbuf);
	}
	fputc('\n', stderr);

	write_scan(outfp, (char**)(inbuf[y%3]));  /* output last scan line */
...
/*
	usage()
        ...
        If the user executes the program with no arguments
...
/*
	old_write_scan() -- Writes a single scan line to the output file.
*/
